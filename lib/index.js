...
const extractValidatorsFromClassElement = (cvDecoratorNames, classElement) => {
    ...
    return classElement.decorators.reduce((acc, decorator) => {
        ...
        console.log(cvDecoratorName.name);
        console.log(lastArg && isOptionsArg(lastArg));
...
const emitSchemasFile = (validationShemas) => {
    ...
    return {
        ...
        content: [
            ...
            validationShemas.map((validationShema) => `registerSchema({
            name: "${validationShema.name}",
            properties: {
              ${Object.entries(validationShema.properties).map(([property, validators]) => `
                ${property}: [
